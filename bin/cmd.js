...
function proc(model, argv) {
  ...
  // prints or sends Invoices
  else if (verb === 'invoicebulk') {
    var generate = function (cid, from, to) {
      var invtpl = require('./invoice.js')(null, conf);

      // get filtered Invoices
      model.Invoice.listByDate(cid, from, to, function(err, invoices, total) {
        if (err) {
          return console.log("Error fetching invoices: " + err);
        }

        console.log("Generating invoices...");
        // pack the generation of each invoice in a promise
        var promises = [];
        invoices.forEach(function (invoice) {
          promises.push(model.Invoice.populateItems(invoice, function (err, inv) {
            debuglog("Invoice row:\n" + JSON.stringify(inv, null, 2) + "\n");
            invtpl.create(inv, inv, false);
          }));
        });

        // wait for all generations and then output info
        Promise.all(promises).then(function () {
          console.log("Done!");
          console.log("Total of all invoices: " + total);
		      var opener = require('opener');
          opener(conf.invoiceDir).unref();
        });
      });
    }
    ...
    if (argv.c || argv.s) {
      getSingleClient(function (err, client) {
        ...
        generate(client.id, argv.from, argv.to);
      });
    }
    else {
      generate(null, argv.from, argv.to);
    }
  }
  else if (verb === 'invoice') {
    if (!argv.c && !argv.s && !argv.from) {
